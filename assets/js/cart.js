...
(function () {
  ...
  function rowMarkup(l) {
    return [
      '<li class="cart-row" data-id="' + l.id + '">',
        '<a href="product.html?id=' + l.id + '"><img class="cart-row__thumb" src="assets/img/' + l.img + '.jpg" alt="' + l.name + '" width="96" height="96" loading="lazy"></a>',
        '<div class="cart-row__info">',
          '<p class="cart-row__cat">' + l.cat + '</p>',
          '<h2 class="cart-row__name"><a href="product.html?id=' + l.id + '">' + l.name + '</a></h2>',
          '<p class="cart-row__unit">' + GG.money(l.price) + '　' + l.unit + '</p>',
          '<div class="qty" role="group" aria-label="' + l.name + ' 數量" style="margin-top:12px">',
            '<button class="qty__btn" type="button" data-dec aria-label="減少數量">−</button>',
            '<input class="qty__val" type="number" inputmode="numeric" value="' + l.qty + '" min="1" max="99" data-qty aria-label="數量">',
